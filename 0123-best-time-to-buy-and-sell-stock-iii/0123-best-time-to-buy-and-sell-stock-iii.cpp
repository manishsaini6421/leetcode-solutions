...
public:
    ...
    int solveMem(int index,int flag,int limit,vector<int>& prices,  vector<vector<vector<int>>> &dp){
        if(index==prices.size())return 0;
        if(limit==0)return 0;

        if(dp[index][flag][limit]!=-1)return dp[index][flag][limit];
        int buy=0;
        int sell=0;
        ...
        if(flag){
            ...
            sell=prices[index]+solveMem(index+1,1,limit-1,prices,dp);
        }
